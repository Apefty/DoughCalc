...
## [2026-08-16] — Correction: preserve {{lang.KEY}} source templates (was being overwritten)

The previous pre-render commit had a bug that defeated the entire point of the `{{lang.KEY}}` approach: `scripts/prerender.js` compiled `biga.html` and then **overwrote that same file** with the rendered (literal-Ukrainian) output, destroying the placeholders. That meant every translated page's source would need re-authoring with `{{lang.KEY}}` syntax again by hand — exactly the "rewrite one word a thousand times" problem the placeholder system exists to avoid.

Fixed by separating source from build output:
- Hand-authored templates now live as `<name>.hbs.html` (e.g. `data/pages/preferments/biga.hbs.html`) — this file is the one you edit, contains `{{lang.KEY}}` placeholders, and `scripts/prerender.js` never writes to it.
- The build compiles each `.hbs.html` into sibling **build artifacts**: `<name>.html` (default locale, uk) and `<name>.<code>.html` (every other locale) — these are the files `cat.js` actually fetches at runtime, regenerated by `npm run build` whenever a `.hbs.html` or a `data/lang/*.js` dictionary changes.
...
## [2026-08-16] — Dictionary fully populated: 285 keys, 0 cards touched

Scanned all 251 card files, extracted every unique untranslated string, drafted key names + EN translations, presented for review (`i18n_dictionary_draft.md`), then merged the approved draft into `ua.js`/`en.js`.

- 86 → **285 keys**, full EN/UA parity, no duplicate key names.
- Breakdown of the addition: 26 section/hub titles (menu-level), 53 recipe-specific card titles, 137 ingredient/field vocabulary entries (relabeled-field text like "Мед", "Волоські горіхи", "Крем-фреш", etc., found across every card).
